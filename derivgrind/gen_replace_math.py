...
class DERIVGRIND_MATH_FUNCTION_BASE:
  def __init__(self,name,type_,math_evals):
    self.name = name
    self.type = type_
    if self.type=="double":
      self.size = 8
      ...
      exit(1)
    self.math_evals = math_evals

class DERIVGRIND_MATH_FUNCTION(DERIVGRIND_MATH_FUNCTION_BASE):
  """Wrap a math.h function to also handle
    the derivative information."""
  def __init__(self,name,deriv,type_,math_evals):
    super().__init__(name,type_)
    self.deriv = deriv
  def c_code(self):
    code = f"""{self.type} I_WRAP_SONAME_FNNAME_ZZ(libmZdsoZa, {self.name}ZAZa) ( {self.type} x ) {{ \n""" 
    original_functions = set()
    # acquire function handles into original libm
    for evaluatedname, function, args, functiontype in math_evals:
      if function not in original_functions:
        code += f"OrigFn {function}; VALGRIND_GET_ORIG_FN({function});\n"
    # use handles
    for evaluatedname, function, args, functiontype in math_evals:
      code += f"double {evaluatedname}; CALL_FN_D_{functiontype}({evaluatedname}, {function}, {args});\n"
    # get dot value of input
    code += f"VALGRIND_GET_DERIVATIVE(&x,&x_d,{self.size});\n"
    # calculation of value
    code += f"{self.type} ret; CALL_FN_D_{functiontype}(ret, {self.name}, x);\n"
    # calculation of dot value
    code += f"{self.type} ret_d = ({self.deriv}) * x_d;\n"
    # set dot value of output
    code += f"VALGRIND_SET_DERIVATIVE(&ret,&ret_d,{self.size});\n"
    code += f"return ret;\n"
    code += f"}}"
