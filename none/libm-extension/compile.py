...
for architecture in ["x86","x86_64"]:
  path = "lib32/" if architecture=="x86" else "lib64/"
  m32if32 = ["-m32"] if architecture=="x86" else []

  # Sample program to find libm.so.
  compile_sample = subprocess.run(["gcc", "sample.c", "-o", path+"sample", "-lm"]+m32if32,universal_newlines=True)
  ...
  ldd_sample = subprocess.run(["ldd", path+"sample"], stdout=subprocess.PIPE,universal_newlines=True)
  libmso_name = None
  libmso_path = None
  ...
  with open(path+"version-script", "w") as f:
    f.write(version_script)

  # Copy original libm.so.
  cp_libm = subprocess.run(["cp", libmso_path, path+"libmoriginal.so"],universal_newlines=True)

  # Produce source file of the library.
  with open(path+"math.c","w") as f:
    f.write("""
      // -------------------------------------------
